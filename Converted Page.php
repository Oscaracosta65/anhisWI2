<?php
/**
 * LottoExpert.net — Wisconsin Super Cash (WI2) Results Intelligence
 * Joomla 5.x | PHP 8.1+ | UTF-8
 *
 * GAME CONFIG IN THIS FILE:
 * - game_id      = WI2
 * - main balls   = 6  (first, second, third, fourth, fifth, sixth)
 * - range        = 01..39  (single pool, no separate bonus)
 * - hasBonus     = false
 *
...
$hasBonus      = false;
$bonusLabel    = '';
$pickLabel     = 'Pick ' . $mainBallCount;

// ── Routes ────────────────────────────────────────────────────────────────────
$routeSkaiUrl   = '/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=' . rawurlencode($gameId);
$routeAiUrl     = '/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=' . rawurlencode($gameId);
$routeSkipHit   = '/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=' . rawurlencode($gameId);
$routeHeatmap   = '/picking-winning-numbers/artificial-intelligence/wisconsin-super-cash-heatmap-analysis';
$routeMcmc      = '/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=' . rawurlencode($gameId);
$routeArchiveBase  = '/lottery-archives-pick6';
$routeLowestBase   = '/lowest-drawn-number-analysis';
...
$_sn = isset($stateName) ? (string) $stateName : '';
$_gn = isset($gName)     ? (string) $gName     : '';

$pageTitle = 'Results Intelligence — ' . $_sn . ' • ' . $_gn . ' | LottoExpert.net';
$pageDesc  = 'Verified draw results, frequency analysis, recency patterns, and advanced SKAI tools for '
           . $_sn . ' ' . $_gn
           . '. Explore the most active numbers, quiet stretches, hot/cold distributions, and full draw history.';
...
function leFindRepeatedFromLatest(array $latestBalls, array $previousRows, array $mainCols): array
{
    $repeated = [];
...
foreach ($rowsMain as $idx => $row) {
    foreach ($mainBallCols as $colName) {
        $ball = trim((string) ($row[$colName] ?? ''));
...
$repeatedNumbers   = leFindRepeatedFromLatest($latestMainBalls, array_slice($rowsMain, 1, 10), $mainBallCols);
$mostActiveSummary = array_slice($topActiveKeys, 0, 3);
$quietSummary      = array_slice($quietestKeys,  0, 3);

// ── Window shift comparison ───────────────────────────────────────────────────
...
$top50  = leTopKeysByValue($counts50,  $mainBallCount, false);
$top300 = leTopKeysByValue($counts300, $mainBallCount, false);
...
if ($drawDate !== '') {
    foreach ($latestMainBalls as $ball) {
        $prevDate = leGetPreviousOccurrenceDate($db, (string) $dbCol, $gameId, $drawDate, $ball, $mainBallCols);
...
$rangeDisplay = lePad2((string) $mainBallMin) . '&ndash;' . lePad2((string) $mainBallMax);

// ── Build archive / utility route suffixes ────────────────────────────────────
$_sTn = isset($stateAbrev) ? strtolower((string) $stateAbrev) : '';
$_archiveSuffix = '?gId=' . rawurlencode($gId)
    . '&amp;stateName=' . rawurlencode($_sn)
    . '&amp;gName='     . rawurlencode($_gn)
    . '&amp;sTn='       . rawurlencode($_sTn);
$_lowestSuffix = $_archiveSuffix;
?>
<style>
:root{
  --skai-blue:#1C66FF;
...
.skai-btn:focus,
.skai-btn:focus-visible{outline:3px solid rgba(255,255,255,.30);outline-offset:3px}
...
@media (max-width:1080px){
  ...
  .skai-tool-grid,
  .skai-overview-grid,
  .skai-two-col{grid-template-columns:1fr}
  .skai-hero-actions,
  .skai-advanced-links{grid-template-columns:1fr}
...
$_jsonLdDataset = [
    '@context'    => 'https://schema.org',
    '@type'       => 'Dataset',
    'name'        => $_gn . ' Frequency and Draw History Analysis',
    'description' => 'Historical draw frequency, recency, and number distribution data for ' . $_sn . ' ' . $_gn . '. Includes hot numbers, quiet numbers, and full Pick ' . $mainBallCount . ' number analysis.',
    'keywords'    => 'lottery, frequency analysis, hot numbers, quiet numbers, draw history, ' . $_sn . ', ' . $_gn . ', pick ' . $mainBallCount,
];

echo '<script type="application/ld+json">' . "\n";
echo json_encode($_jsonLdPage, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
echo "\n" . '</script>' . "\n";

echo '<script type="application/ld+json">' . "\n";
echo json_encode($_jsonLdDataset, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
echo "\n" . '</script>' . "\n";
?>
...
<div class="skai-page">
    ...
    <div class="skai-hero-inner">
      <div class="skai-hero-top">
        ...
        <div class="skai-logo" aria-hidden="<?php echo ($logo['exists']) ? 'false' : 'true'; ?>">
          <?php if ($logo['exists'] && $logo['url'] !== '') : ?>
            <img
              ...
              alt="<?php echo htmlspecialchars($_sn . ' ' . $_gn, ENT_QUOTES, 'UTF-8'); ?>"
              width="110"
              height="110"
              loading="lazy"
              decoding="async"
        ...
        <div class="skai-hero-copy">
          <div class="skai-kicker">Results Intelligence &bull; Verified Draw &bull; <?php echo htmlspecialchars($pickLabel, ENT_QUOTES, 'UTF-8'); ?> Analysis</div>
          <h1 class="skai-title">
            <?php echo htmlspecialchars($_sn, ENT_QUOTES, 'UTF-8'); ?>
            &ndash;
            <?php echo htmlspecialchars($_gn, ENT_QUOTES, 'UTF-8'); ?>
          </h1>
          ...
          <div class="skai-ball-row" aria-label="Latest drawn numbers">
            <?php foreach ($latestMainBalls as $_ball) : ?>
              <span class="skai-ball skai-ball--main"><?php echo htmlspecialchars(lePad2($_ball), ENT_QUOTES, 'UTF-8'); ?></span>
            <?php endforeach; ?>
          </div>

          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="<?php echo htmlspecialchars($routeSkaiUrl, ENT_QUOTES, 'UTF-8'); ?>">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="<?php echo htmlspecialchars($routeAiUrl, ENT_QUOTES, 'UTF-8'); ?>">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($routeSkipHit, ENT_QUOTES, 'UTF-8'); ?>">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($routeMcmc, ENT_QUOTES, 'UTF-8'); ?>">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($routeHeatmap, ENT_QUOTES, 'UTF-8'); ?>">Heatmap Analysis</a>
          </div>
        </div>
        ...
        <div class="skai-card">
          ...
          <div class="skai-card-body">
            <div class="skai-history-list">
              <?php foreach ($drawHistoryRows as $_histRow) : ?>
                <div class="skai-history-item">
                  <div class="skai-history-name"><?php echo htmlspecialchars((string) $_histRow['label'], ENT_QUOTES, 'UTF-8'); ?></div>
                  <div class="skai-history-date">
                    <?php if (!empty($_histRow['prevDate'])) : ?>
                      Previously seen on <?php echo htmlspecialchars(leFmtDateLong((string) $_histRow['prevDate']), ENT_QUOTES, 'UTF-8'); ?>
                    <?php else : ?>
                      No previous appearance found in the loaded historical set
                    <?php endif; ?>
                  </div>
                  <div class="skai-history-badge">
                    <?php echo ($_histRow['drawsAgo'] !== null) ? (int) $_histRow['drawsAgo'] . ' drws ago' : '&mdash;'; ?>
                  </div>
                </div>
              <?php endforeach; ?>
            </div>
          ...
          <div class="skai-card-body">
            <div class="skai-window-shift">
              ...
              <div class="skai-shift-panel">
                ...
                <p class="skai-shift-text"><?php echo htmlspecialchars(leCommaList(array_slice($top50, 0, $mainBallCount)), ENT_QUOTES, 'UTF-8'); ?></p>
              </div>

              <div class="skai-shift-panel">
                <p class="skai-shift-label">Broader 300-draw leaders</p>
                <p class="skai-shift-text"><?php echo htmlspecialchars(leCommaList(array_slice($top300, 0, $mainBallCount)), ENT_QUOTES, 'UTF-8'); ?></p>
              </div>
            </div>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The panels below show the complete main-number distribution across all values <?php echo $rangeDisplay; ?> and the recency distance for each number in the selected window.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values <?php echo $rangeDisplay; ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--ember">
            ...
            <span class="skai-card-sub">Distance since last appearance for each main number in the <?php echo (int) $nodCurrentMain; ?>-draw window</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
              <div class="skai-chart-frame skai-chart-frame--tall">
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. A larger draws-ago value means the number has not appeared recently within the selected window.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    ...
    <div class="skai-controls">
      <form name="fqsearch" method="post" action="/all-us-lotteries/results-analysis?st=<?php echo htmlspecialchars((string) ($stateAbrev ?? ''), ENT_QUOTES, 'UTF-8'); ?>&amp;stn=<?php echo htmlspecialchars($_sn, ENT_QUOTES, 'UTF-8'); ?>&amp;gm=<?php echo htmlspecialchars($_gn, ENT_QUOTES, 'UTF-8'); ?>#tables">
        <div class="skai-controls-row">
          <div class="skai-controls-left">
            ...
            <select name="nod" id="nod" class="skai-select">
              <?php foreach (range(10, 700, 5) as $_opt) : ?>
                <option value="<?php echo (int) $_opt; ?>"<?php echo ((int) $_opt === (int) $nodCurrentMain) ? ' selected="selected"' : ''; ?>>
                  <?php echo (int) $_opt; ?>
                </option>
              <?php endforeach; ?>
            </select>
    ...
    <div class="skai-section-body">
      <div class="skai-card">
        <div class="skai-card-head skai-card-head--horizon">
          Main numbers table
          <span class="skai-card-sub">Exact counts and recency for all values <?php echo $rangeDisplay; ?></span>
        </div>
        ...
        <div class="skai-table-wrap">
          <table id="skai-main-table" class="skai-table" aria-label="Main numbers frequency table">
            ...
            <tbody>
              <?php for ($_i = $mainBallMin; $_i <= $mainBallMax; $_i++) : ?>
                <?php
                $_num    = ($_i < 10) ? '0' . $_i : (string) $_i;
                $_cnt    = (int) ($mainCounts[$_num] ?? 0);
                [$_sort, $_lbl] = leDrawingsAgoLabel($mainLastSeenIndex[$_num] ?? null, (int) $nodCurrentMain);

                $_rowClass = 'all';
                if (in_array($_num, $topActiveKeys, true)) {
                    $_rowClass .= ' active';
                }
                if (in_array($_num, $quietestKeys, true)) {
                    $_rowClass .= ' quiet';
                }
                if (($mainLastSeenIndex[$_num] ?? null) !== null && (int) $mainLastSeenIndex[$_num] <= 4) {
                    $_rowClass .= ' recent';
                }
                ?>
                <tr data-tags="<?php echo htmlspecialchars($_rowClass, ENT_QUOTES, 'UTF-8'); ?>">
                  <td><span class="skai-pill skai-pill--main"><?php echo htmlspecialchars($_num, ENT_QUOTES, 'UTF-8'); ?></span></td>
                  <td><?php echo (int) $_cnt; ?> X</td>
                  <td data-sort="<?php echo (int) $_sort; ?>"><?php echo htmlspecialchars($_lbl, ENT_QUOTES, 'UTF-8'); ?></td>
                  <td>
                    <input
                      class="skai-checkbox js-track-main"
                      type="checkbox"
                      value="<?php echo htmlspecialchars($_num, ENT_QUOTES, 'UTF-8'); ?>"
                      aria-label="Track main number <?php echo htmlspecialchars($_num, ENT_QUOTES, 'UTF-8'); ?>"
                    >
                  </td>
                </tr>
              <?php endfor; ?>
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($routeSkaiUrl, ENT_QUOTES, 'UTF-8'); ?>">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($routeAiUrl, ENT_QUOTES, 'UTF-8'); ?>">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($routeSkipHit, ENT_QUOTES, 'UTF-8'); ?>">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($routeMcmc, ENT_QUOTES, 'UTF-8'); ?>">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($routeHeatmap, ENT_QUOTES, 'UTF-8'); ?>">Heatmap Analysis</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($routeArchiveBase . $_archiveSuffix, ENT_QUOTES, 'UTF-8'); ?>">Lottery Archives</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($routeLowestBase . $_lowestSuffix, ENT_QUOTES, 'UTF-8'); ?>">Lowest Number Analysis</a>
      </div>
    </div>
  </section>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. The purpose of this page is to make the recent behavior of the game easier to understand, compare, and carry into deeper SKAI analysis. <?php echo htmlspecialchars($_sn . ' ' . $_gn, ENT_QUOTES, 'UTF-8'); ?> draws <?php echo (int) $mainBallCount; ?> numbers from a single pool of <?php echo htmlspecialchars(lePad2((string) $mainBallMin) . '&ndash;' . lePad2((string) $mainBallMax), ENT_QUOTES, 'UTF-8'); ?>. All analysis on this page reflects that structure.
      </div>
    </div>
  </section>
...
(function () {
  ...
  function renderCharts() {
    ...
    var topActiveCanvas = safeGetCanvas('topActiveChart');
    var quietCanvas     = safeGetCanvas('quietChart');
    var fullMainCanvas  = safeGetCanvas('fullMainChart');
    var recencyCanvas   = safeGetCanvas('recencyChart');

    if (topActiveCanvas && !_chartInstances['topActiveChart']) {
      _chartInstances['topActiveChart'] = new Chart(topActiveCanvas.getContext('2d'), {
        type: 'bar',
        data: {
    ...
    if (quietCanvas && !_chartInstances['quietChart']) {
      _chartInstances['quietChart'] = new Chart(quietCanvas.getContext('2d'), {
        type: 'bar',
        data: {
    ...
    if (fullMainCanvas && !_chartInstances['fullMainChart']) {
      _chartInstances['fullMainChart'] = new Chart(fullMainCanvas.getContext('2d'), {
        type: 'bar',
        data: {
    ...
    if (recencyCanvas && !_chartInstances['recencyChart']) {
      _chartInstances['recencyChart'] = new Chart(recencyCanvas.getContext('2d'), {
        type: 'bar',
        data: {
          labels: chartData.mainLabels,
          datasets: [{
  ...
  function bindFilters() {
    ...
    function applyFilter(filter) {
      ...
      for (j = 0; j < rows.length; j++) {
        var tags = rows[j].getAttribute('data-tags') || '';
